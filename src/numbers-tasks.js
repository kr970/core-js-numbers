...
function roundToPowerOfTen(num, pow) {
  ...
  const round = Math.round(num / divider);
...
function isNumber(number) {
  const numberCheck = typeof number === 'number';
  const isNaNCheck = !Number.isNaN(number);
  const infinityCheck = Number.isFinite(number);
  return numberCheck && isNaNCheck && infinityCheck;
}
